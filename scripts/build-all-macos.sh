...
echo "🍎 Building Mini MSP Agent for macOS..."
...
build_component() {
    ...
    case $component in
        ...
        "server")
            ...
            ;;
        "qt_client")
            echo "🔧 Building Qt Client for macOS..."
            cd apps/qt_client
            mkdir -p build
            cd build
        ...
        "shared")
            ...
            ;;
        "plugins")
            echo "🔧 Building C++ plugins for macOS..."
            mkdir -p plugins/build
            cd plugins
            cmake -S . -B build -A "x86_64;arm64"
...
echo ""
echo "🚀 Run with: ./scripts/start-macos.sh"
