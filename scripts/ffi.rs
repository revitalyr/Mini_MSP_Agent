...
pub struct FileContent {
    ...
    pub size: usize,
    pub success: u32, // Changed from bool to u32 for stable FFI size
...
impl FileContentData {
    unsafe fn from_c_struct(content: &FileContent) -> Self {
        Self {
            ...
            size: content.size,
            success: content.success != 0, // Convert u32 to bool
...
impl CommandResultData {
    unsafe fn from_c_struct(result: &CommandResult) -> Self {
        Self {
            ...
            success: result.success != 0, // Convert u32 to bool
...
mod tests {
    ...
    fn test_ffi_layout_consistency() {
        ...
        // FileContent
        assert_eq!(size_of::<FileContent>(), 280);
        assert_eq!(offset_of!(FileContent, success), 0);
        ...
        // CommandResult
        assert_eq!(size_of::<CommandResult>(), 280);
        assert_eq!(offset_of!(CommandResult, success), 0);
    }
