 ...
 */
typedef struct {
    uint32_t success; // Changed from bool to uint32_t for stable FFI size
    char* content;
    size_t size;
    char error[256];
} file_content_t;
...
static_assert(sizeof(file_content_t) == 280, "file_content_t size must be 280");
static_assert(sizeof(command_result_t) == 280, "command_result_t size must be 280");
...
static_assert(offsetof(file_content_t, success) == 0);
static_assert(offsetof(command_result_t, success) == 0);
static_assert(offsetof(command_result_t, exit_code) == 4);
