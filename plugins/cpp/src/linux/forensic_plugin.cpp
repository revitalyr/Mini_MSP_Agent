...
static bool get_system_info_impl(system_info_t* info) {
    ...
    strncpy(info->os_type, "Linux", kMaxOsTypeLen - 1);
    
    // Get kernel version
    struct utsname un;
    if (uname(&un) == 0) {
        strncpy(info->os_version, un.release, kMaxOsVersionLen - 1);
    }
...
static char* execute_json_impl(const char* json_request) {
    ...
    if (req.find("\"cmd\":\"get_forensic\"") != std::string::npos || 
        ...
        response = "{\"status\":\"error\",\"message\":\"Unknown or malformed command\"}";
    }
