//! Custom Plugin Template for Mini MSP Agent (Linux)
//!
...
bool execute_custom_command(const char* command, char* output, size_t output_size) {
    ...
    if (strcmp(command, "hello") == 0) {
        strncpy(output, "Hello from custom plugin!", output_size - 1);
    ...
    // Unknown command
    strncpy(output, "Unknown command", output_size - 1);
    output[output_size - 1] = '\0';
    return false;
}
...
bool get_custom_metrics(void* metrics_buffer, size_t buffer_size) {
    ...
    strncpy(metrics->status, "active", sizeof(metrics->status) - 1);
