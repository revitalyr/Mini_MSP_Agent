...
static bool execute_custom_command(const char* command, CommandResult* result) {
    ...
    if (strncmp(command, "echo ", 5) == 0) {
        ...
        strncpy(result->output, "pong", sizeof(result->output));
        result->success = true;
        result->error[0] = '\0';
    }
    else if (strncmp(command, "config ", 7) == 0) {
        // Config command placeholder
        strncpy(result->output, "Configuration updated", sizeof(result->output));
        result->success = true;
        result->error[0] = '\0';
    }
    else {
...
static bool get_custom_metrics(CustomMetrics* metrics) {
    ...
    strncpy(metrics->status, plugin_state.initialized ? "running" : "stopped",
            sizeof(metrics->status));
